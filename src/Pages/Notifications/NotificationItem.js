...
export const NotificationItem = ({ item, handleDelete, index }) => {
 ...
 return (
  <div
    className={`min-h-[60px] sm:h-[60px] flex flex-row justify-between items-center w-full p-3 sm:p-3 ${
      index % 2 === 0 ? "bg-[#4071B60D]" : "bg-white"
    }`}
  >
      {!item.read && (
        <span className="w-[10px] h-[10px] bg-[#4071B6] rounded-full inline-block"></span>
      )}
    {/* Left side: Dot + Message */}
    <div className="flex items-center gap-2 pl-0 sm:pl-6 text-sm sm:text-base text-gray-900 flex-1 w-full sm:w-auto">
    
      <span>{item.message}</span>
    </div>

    {/* Right side: Time + Delete */}
    <div className="flex items-center justify-end w-auto gap-2 sm:gap-3">
