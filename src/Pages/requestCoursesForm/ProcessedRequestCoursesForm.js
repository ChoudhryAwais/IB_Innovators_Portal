          ...
          <Accordion key={index} className="border border-gray-200 rounded-lg shadow-sm">
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <div className="flex flex-1 justify-between items-center">
                <span className="text-gray-800 font-medium">{item.userDetails?.email}</span>
              </div>
            </AccordionSummary>
            <AccordionDetails className="bg-gray-50 rounded-lg p-4">
              <div className="space-y-4">
                <div className="border-b border-gray-300 pb-4">
                  ...
                  <div><strong>End Date & Time:</strong> {formatDateTime(item?.endDateAndTime)}</div>
                  <div><strong>Start Date & Time:</strong> {formatDateTime(item?.startDateAndTime)}</div>
                  <div><strong>Final Exam Date:</strong> {item?.finalExamDate}</div>
                  <div>
                    <strong>Objective:</strong>
                    <ul className="list-disc list-inside ml-4">
                      {item.objective?.map((i, idx) => (
                        <li key={idx}>{i}</li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    ...
                    <ul className="list-disc list-inside ml-4">
                      {item.seekingTutoringFor?.map((i, idx) => (
                        <li key={idx}>{i}</li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    ...
                    <ul className="list-disc list-inside ml-4">
                      {item?.howOftenToTakeLesson?.map((i, idx) => (
                        <li key={idx}>{i}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  {item?.wantGuidanceAndSupport === "yes" && (
                    <>
                      <div>
                        ...
                        <ul className="list-disc list-inside ml-4">
                          {item?.guidanceAndSupportSubjects?.map((i, idx) => (
                            <li key={idx}>{i}</li>
                          ))}
                        </ul>
                      </div>
