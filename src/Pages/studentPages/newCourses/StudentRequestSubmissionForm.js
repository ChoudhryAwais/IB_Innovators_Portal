...
export function StudentRequestSubmissionForm({ item, handleClose }) {
  const { userDetails, subjectsArray, addNotification, adminAddNotification } =
  ...
  async function submittingForm() {
    ...
    try {
      ...
      await updateDoc(doc(db, "studentRequests", docId), { id: docId });

      addNotification(
        `You requested a course for ${subject}.`,
        userDetails?.userId
      );
      adminAddNotification(
        `${userDetails?.userName} requested a course for ${subject}.`
      );
