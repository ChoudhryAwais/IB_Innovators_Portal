  ...
  const tabs = [
    {
      ...
      label: "Create Job",
      icon: (
        <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M1 6H21M5 11H9M5 15H9M14.2361 15.2764L17 13.8944C17.737 13.5259 17.737 12.4741 17 12.1056L14.2361 10.7236C13.5712 10.3912 12.7889 10.8747 12.7889 11.618V14.382C12.7889 15.1253 13.5712 15.6088 14.2361 15.2764ZM5 21H17C19.2091 21 21 19.2091 21 17V5C21 2.79086 19.2091 1 17 1H5C2.79086 1 1 2.79086 1 5V17C1 19.2091 2.79086 21 5 21Z" stroke="#28303F" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      ),
      activeIcon: (
    ...
    },
    {
      ...
      icon: (
        <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M12 19V22M12 22H9M12 22H15M14 8C14 6.89543 13.1046 6 12 6C10.8954 6 10 6.89543 10 8C10 9.10457 10.8954 10 12 10C13.1046 10 14 9.10457 14 8ZM6 19H18C20.2091 19 22 17.2091 22 15V6C22 3.79086 20.2091 2 18 2H6C3.79086 2 2 3.79086 2 6V15C2 17.2091 3.79086 19 6 19ZM9.5 15H14.5C15.3284 15 16 14.3284 16 13.5C16 12.6716 15.3284 12 14.5 12H9.5C8.67157 12 8 12.6716 8 13.5C8 14.3284 8.67157 15 9.5 15Z" stroke="#28303F" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      ),
      activeIcon: (
        <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    },
    {
      ...
      icon: (
        <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M2 15V6C2 3.79086 3.79086 2 6 2H18C20.2091 2 22 3.79086 22 6V15M2 15C2 17.2091 3.79086 19 6 19H18C20.2091 19 22 17.2091 22 15M2 15H22M12 19V22M12 22H9M12 22H15M7 7H12M7 11H17" stroke="#28303F" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      ),
      activeIcon: (
        <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <TopHeadingProvider>
      <div className="min-h-screen p-4 sm:p-6">
        <div className="mr-0 sm:mr-[10px] mb-4 sm:mb-5 pt-0">
          <div className="bg-white rounded-lg border border-gray-200">
            {/* Tabs header */}
            <div className="flex flex-col sm:flex-row border-b border-gray-200 mx-0 sm:mx-4 overflow-x-auto">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center justify-center sm:justify-start gap-2 px-3 sm:px-6 py-3 text-sm sm:text-[16px] transition-colors duration-200 border-b-2 sm:border-b-2 flex-1 sm:flex-none min-w-0 ${activeTab === tab.id
                    ? "text-[#4071B6] border-[#4071B6] font-semibold bg-[#4071B60D] sm:bg-transparent"
                    : "text-[#16151C] border-transparent font-light hover:bg-gray-50 sm:hover:border-gray-300"
                    }`}
                >
