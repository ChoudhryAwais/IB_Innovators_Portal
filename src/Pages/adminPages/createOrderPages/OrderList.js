...
export function OrderList() {
  ...
  const visiblePages = getVisiblePages(
    currentPage,
    Math.ceil(searchedData.length / itemsPerPage)
  );
  ...
  return (
    <div className="flex-1 min-h-screen">
      {data.length !== 0 && (
        <div className="relative mb-4 sm:mb-6">
          ...
          <input
            ...
            className="w-full h-[40px] sm:h-[50px] pl-8 sm:pl-10 pr-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white text-sm sm:text-base"
            defaultValue=""
          />
        </div>
      )}
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-600 mt-2 pl-0 sm:pl-[4.5rem] space-y-1">
                  <div className="flex flex-col sm:flex-row">
                    <span className="w-28 sm:w-40 font-light text-[#16151C] text-[12px] sm:text-[14px] mb-1 sm:mb-0">Order ID:</span>
                    <span className="text-[#16151C] text-[12px] sm:text-[14px] font-medium truncate">{item?.id}</span>
                  </div>
                  <div className="flex flex-col sm:flex-row">
                    <span className="w-28 sm:w-40 font-light text-[#16151C] text-[12px] sm:text-[14px] mb-1 sm:mb-0">Country:</span>
                    <span className="text-[#16151C] text-[12px] sm:text-[14px] font-medium">{item?.country}</span>
                  </div>
                  <div className="flex flex-col sm:flex-row">
                    <span className="w-28 sm:w-40 font-light text-[#16151C] text-[12px] sm:text-[14px] mb-1 sm:mb-0">Course Requested:</span>
                    <span className="text-[#16151C] text-[12px] sm:text-[14px] font-medium">{item?.subject}</span>
                  </div>
                </div>
              </div>
      ...
      {searchedData?.length > itemsPerPage && (
        <div className="flex flex-col sm:flex-row items-center justify-between mt-4 sm:mt-6 px-3 sm:px-4 py-3 bg-white gap-3 sm:gap-0">
          ...
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            {/* Page numbers */}
            {getVisiblePages(currentPage, Math.ceil(searchedData.length / itemsPerPage), 4).map((page) => (
              <Button
                key={page}
      ...
      {searchedData?.length === 0 && (
        <div className="flex-1 text-center text-gray-400 text-lg sm:text-xl py-8 sm:py-12">No Orders</div>
      )}
