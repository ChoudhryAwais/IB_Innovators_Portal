...
const hasDatePassed = (date, time, timezone) => {
  // Combine date and time strings
  const dateTimeString = `${date} ${time}`;
  ...
  // Get the current time in the same timezone as the input date
...
export function UpcomingAndCanceledSessions() {
  ...
  const { addNotification, userDetails, convertToGBP } = useContext(MyContext);
  ...
  const upcomingDisplayedSessions = upcomingSessions?.slice(
    upcomingStartIndex,
    upcomingEndIndex
  );
  ...
  useEffect(() => {
    if (selectedLink?.date && selectedLink?.time && selectedLink?.note && selectedLink?.duration && selectedLink?.price && selectedLink?.tutorHourlyRate) {
      setSessionData({
        date: selectedLink?.date,
          time: selectedLink?.time,
  ...
  const completeSession = async (linkId) => {
    if (sessionReview !== "") {
      try {
        setSubmitting(true);
        const studentRef = doc(db, "Linked", linkId.linkId);
        ...
        if (studentSnapshot.exists()) {
          ...
          // Filter out the session you want to remove
          const updatedBookedSessions = bookedSessions.filter(
            (session) => session.id !== linkId.id
          );
          ...
          const docRef = doc(collection(db, "Linked"), selectedLink.linkId);
          ...
          await addNotification(
            `You completed a session with ${selectedLink.studentName} for ${selectedLink.subject} on ${sessionData.date} | ${sessionData.time}.`,
            userDetails?.userId
          );
          await addNotification(
            `${selectedLink.teacherName} completed the session with you for ${selectedLink.subject} on ${sessionData.date} | ${sessionData.time}.`,
  ...
  const editSession = async (linkId) => {
    ...
    try {
      ...
      const updatedSessionInfo = bookedSessions.map((session) =>
        session.id === linkId.id
          ? { ...session, ...sessionData, price: selectedLink.price, tutorHourlyRate: selectedLink?.tutorHourlyRate }
          : session
      );

      // Update the document with the new session info
      await updateDoc(studentRef, {
        bookedSessions: updatedSessionInfo,
        isSessionBooked: true,
      });

      await addNotification(
        `You updated a session with ${selectedLink.studentName} for ${selectedLink.subject} on ${sessionData.date} | ${sessionData.time}.`,
        userDetails.userId
      );
      await addNotification(
        `${selectedLink.teacherName} updated the session with you for ${selectedLink.subject} on ${sessionData.date} | ${sessionData.time}.`,
        selectedLink.studentId
      );
  ...
  const deleteSession = async (linkId) => {
    try {
      ...
      const studentNotification = `${selectedLink.teacherName} cancelled the session with you for ${selectedLink.subject} on ${sessionData.date} | ${sessionData.time}.`;
      const teacherNotification = `You cancelled a session with ${selectedLink.studentName} for ${selectedLink.subject} on ${sessionData.date} | ${sessionData.time}.`;
          ...
          <>
            {upcomingDisplayedSessions?.map((item, index) => (
              ...
              >
                ...
                >
                  <div
                    style={{ fontSize: "small", flex: 0.1, textAlign: "right" }}
                  >
                ...
                >
                  ...
                  <Button
                    style={{ flex: 1 }}
                    disabled={hasDatePassed(item?.date, item?.time, userDetails?.timeZone)}
                    onClick={() => {
          ...
          <>
            {displayedSessions?.map((item, index) => (
              ...
              >
                <div
                  style={{ fontSize: "small", flex: 0.1, textAlign: "right" }}
                >
                  {formatDateInUserTimezone(item?.date, userDetails?.timeZone)}
                </div>
            ...
            {allSessions?.length > itemsPerPage && (
              ...
              >
                <Stack spacing={2}>
                  <Pagination
                    count={Math.ceil(allSessions?.length / itemsPerPage)}
                    page={currentPage}
          ...
          <div style={{ marginTop: "20px", padding: "10px" }}>
            ...
            >
              <div style={{ flex: 1, width: "100%", marginBottom: "5px" }}>
                ...
                />
                <div style={{flex: 1, textAlign: 'right'}}>{sessionReview?.length} / 100</div>
              </div>
            </form>
            <div
              style={{
